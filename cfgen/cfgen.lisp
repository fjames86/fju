...
(defun emit-cmd-list (entries)
  ...
  (dolist (entry entries)
    ...
    (format t "            printf( \"%-16s %-4d: TAG=%\"PRIx64\" ~{~A=%~A ~}\\n\", \"~A\", i, lst[i].tag~{, lst[i].~A~} );~%" 
	    (mapcan (lambda (field)
		      (list (field-name field)
			    (if (eq (field-type field) :string) "s" "d")))
		    (entry-fields entry))
	    (entry-name entry)
...
(defun emit-main (entries extras)
  (labels ((emit-set-parser (entry)
	   ...
	   (let ((field (car (entry-fields entry))))
	     (format t "                 if( strcmp( argname, \"~A\" ) == 0 ) {~%" (field-name field))
	     (if (eq (field-type field) :string)
		 (format t "                      if( argval ) strncpy( entry.~A, argval, sizeof(entry.~A) );~%"
			 (field-name field)
			 (field-name field))
		 (format t "                      if( argval ) entry.~A = strtoul( argval, NULL, 10 );~%" (field-name field))))
	   (dolist (field (cdr (entry-fields entry)))
	     (format t "                } else if( strcmp( argname, \"~A\" ) == 0 ) {~%" (field-name field))
	     (if (eq (field-type field) :string)
		 (format t "                      if( argval ) strncpy( entry.~A, argval, sizeof(entry.~A) );~%" (field-name field) (field-name field))
		 (format t "                      if( argval ) entry.~A = strtoul( argval, NULL, 10 );~%" (field-name field))))
	   (format t "                 } else { printf( \"xxx=%s\\n\", argname ); usage( NULL ); }~%")
	   (format t "                 i++;~%")
	   (format t "            }~%"))	   
	 (emit-add-body (entry)
