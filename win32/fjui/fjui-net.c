...
static void fvmlist_cb( struct xdr_s *xdr, struct hrauth_call *hcallp ) {	
	...
	while( b ) {
		...
		xdr_decode_uint32( xdr, &info->modules[info->nmodule].flags );
	    ...
	    for( i = 0; i < info->modules[info->nmodule].nprocs; i++ ) {
			...
			xdr_decode_uint64( xdr, &info->modules[info->nmodule].procs[i].perfdata.nsteps );
			xdr_decode_uint64( xdr, &info->modules[info->nmodule].procs[i].perfdata.rcount );			
		}
