...
static int dmb_call_publish( uint64_t hostid, uint64_t entryid, uint64_t seq, uint32_t msgid, uint32_t flags, char *buf, int len ) {
...
static void dmb_iter_cb( struct rpc_iterator *iter ) {
  ...
  for( i = 0; i < glob.nhost; i++ ) {
    ...
    if( glob.host[i].sentseq < entry.seq ) {
      ...
      sts = dmb_call_publish( glob.host[i].hostid, entry.id, entry.seq, hdr.msgid, hdr.flags, msgbuf, iov[1].len );
...
static int dmb_proc_publish( struct rpc_inc *inc ) {
  ...
  dmb_log( LOG_LVL_INFO, "dmb recv hostid=%"PRIx64" seq=%"PRIu64" msgid=%u len=%u", hostid, seq, msgid, lenp );
  
  sc = glob.sc;
  while( sc ) {
    sc->cb( hostid, seq, msgid, bufp, lenp );
    sc = sc->next;
  }
...
int dmb_open( void ) {
  ...
  while( !sts ) {
    if( ((entry.flags & FREG_TYPE_MASK) == FREG_TYPE_KEY) && (glob.nfvmsc < DMB_MAX_FVMSC) ) {
      ...
      if( !sts ) {
	dmb_log( LOG_LVL_INFO, "dmb init modname=%s procname=%s", glob.fvmsc[glob.nfvmsc].modname, glob.fvmsc[glob.nfvmsc].procname );
